...
export const isValue = PropTypes.oneOfType([PropTypes.instanceOf(Date), PropTypes.arrayOf(PropTypes.instanceOf(Date))]);
...
export const isView = (props, propName, componentName) => {
	...
	if (allowedViews.indexOf(view) === -1) {
		return new Error(
			`Invalid prop \`${propName}\` of value \`${view}\` supplied to \`${componentName}\`, expected one of [${["a", "b", "c", "d", "e"]
				.map(a => `"${a}"`)
				.join(", ")}].`
		);
	}
